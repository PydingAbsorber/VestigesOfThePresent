...
public class VestigeCap implements INBTSerializable<CompoundTag> {
    ...
    public static final Codec<VestigeCap> CODEC = CompoundTag.CODEC.xmap(
            tag -> {
                VestigeCap cap = new VestigeCap();
                cap.deserializeNBT(null, tag);
                return cap;
            },
            cap -> cap.serializeNBT(null)
    );

    public void setSomeData(int value) { this.someData = value; }
    public int getSomeData() { return this.someData; }

    @Override
    public CompoundTag serializeNBT(HolderLookup.Provider provider) {
        CompoundTag nbt = new CompoundTag();
        nbt.putInt("someData", someData);
        saveNBT(nbt);
        return nbt;
    }
    ...
    public void deserializeNBT(HolderLookup.Provider provider, CompoundTag nbt) {
        this.someData = nbt.getInt("someData");
        loadNBT(nbt);
    }
    ...
    public void setChance(){
        this.chance += ConfigHandler.stellarChanceIncrease.get();
    }
    ...
    public void copyNBT(VestigeCap source){
    ...
    public void sendLore(Player player, int number){
        if(player instanceof ServerPlayer serverPlayer && ConfigHandler.lore.get())
    ...
    public ItemStack vestige(int vp, Player player){
        ...
        if(random.nextDouble() < VPUtil.getChance(stellarChance/100,player)){
            ...
            setChance(ConfigHandler.stellarChanceIncrease.get());
            addStellarChallenge(player,vp);
            if(random.nextDouble() < VPUtil.getChance(ConfigHandler.refresherChance.get(),player))
