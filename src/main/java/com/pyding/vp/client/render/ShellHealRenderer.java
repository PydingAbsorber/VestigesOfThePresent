...
public class ShellHealRenderer extends EntityRenderer<ShellHealEntity> {
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(VestigesOfThePresent.MODID,"textures/item/models/shellheal_texture.png");
    public ShellHealRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(ShellHealEntity p_114482_) {
        return TEXTURE;
    }

    @OnlyIn(Dist.CLIENT)
    @Override
    public void render(ShellHealEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
        float scale = 1;
