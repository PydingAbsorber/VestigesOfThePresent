...
public class VipActivator extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand p_41434_) {
        ...
        if(VPUtil.getTag(stack).hasUUID("VPPlayer") && VPUtil.getTag(stack).getUUID("VPPlayer").compareTo(player.getUUID()) != 0){
            player.sendSystemMessage(Component.literal("It's not yours."));
            return super.use(level, player, p_41434_);
        }
        VestigeCap cap = VPUtil.getCap(player);
        cap.setVip((day*VPUtil.getTag(stack).getInt("VPDays"))+System.currentTimeMillis());
        cap.sync(player);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> components, TooltipFlag flag) {
        ...
        components.add(GradientUtil.goldenGradient(Component.translatable("item.vip.desc1",VPUtil.getTag(stack).getInt("VPDays")).getString()));
        ...
        if(VPUtil.getTag(stack).hasUUID("VPPlayer"))
