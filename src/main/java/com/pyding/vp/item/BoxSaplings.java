...
public class BoxSaplings extends Item {
    public BoxSaplings(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand p_41434_) {
        if(!level.isClientSide) {
            ...
            for (Item stack1 : VPUtil.items) {
                if (stack1.getDescriptionId().contains("sapling") || stack1.getDescriptionId().contains("seed"))
                    list.add(stack1);
            }
            if (list.isEmpty())
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> components, TooltipFlag flag) {
        components.add(Component.translatable("box.sapling.info.1").withStyle(ChatFormatting.LIGHT_PURPLE));
    }
}
